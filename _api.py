...
class OctoBotApi:
    ...
    async def get_group_md(self, group_no: str) -> dict[str, Any] | None:
        """获取群的 GROUP.md 内容。

        GET /v1/bot/groups/{groupNo}/md

        返回:
          {"content": "...", "version": 1, "updated_at": "...", "updated_by": "..."}
          群没有 GROUP.md（404）或请求失败时返回 None。
        """
        session = await self._ensure_session()
        url = f"{self.api_url}/v1/bot/groups/{group_no}/md"
        try:
            async with session.get(url) as resp:
                if resp.status == 404:
                    return None
                if resp.status != 200:
                    logger.warning(f"[octo] get_group_md({group_no}) 失败: HTTP {resp.status}")
                    return None
                data = await resp.json()
            ...
            logger.warning(f"[octo] get_group_md({group_no}) 异常: {e}")
            return None
