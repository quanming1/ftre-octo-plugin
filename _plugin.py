...
from _channel import OctoChannel, get_group_md_content
from _tools import create_octo_management_tool

logger = logging.getLogger("ftre.plugin.octo_channel")


class OctoPluginConfig(BaseModel):
    """Validated Octo entry config while preserving channel-specific extensions."""
...
class OctoChannelPlugin(Plugin):  # type: ignore[misc]
    ...
    name = "octo_channel"
    version = "1.0.0"
    inject = ("bus", "session_manager", "channel_manager")
    Config = OctoPluginConfig

    async def setup(self, ctx: Any, config: OctoPluginConfig) -> None:
        """插件初始化：注册 Channel 和 Hook。"""
        config_data = config.model_dump()
        bots = config_data.get("bots", [])
        logger.info(
            f"[octo] 插件初始化: api_url={config_data.get('api_url')} bots={len(bots)}"
        )
        ...
        ctx.register_channel(channel)
        self._channel = channel
        logger.info("[octo] Channel 已注册到 ChannelManager")

        ctx.on(BEFORE_AGENT_RUN, self._on_agent_run)
    ...
    async def _on_agent_run(self, ctx: Any) -> Any:
        ...
        if (
            channel is not None
            and channel._bots
            and ctx.agent_tool_registry is not None
            and "octo_management" not in ctx.agent_tool_registry.names
        ):
            first_api = next(iter(channel._bots.values()))["api"]
            ctx.agent_tool_registry.register(
                create_octo_management_tool(first_api, channel=channel)
            )
        ...
        if isinstance(ctx.messages, list):
            ...
            logger.info(
                f"[octo] Hook: 已注入 Octo 身份提示{' + GROUP.md' if group_md_hint else ''}, 消息数={len(ctx.messages)}"
            )
        return ctx
    ...
    async def _build_group_md_hint(self, session_id: str) -> str:
        ...
        channel = getattr(self, "_channel", None)
        if not session_id or channel is None or not channel.session_manager:
            return ""

        external = await channel.session_manager.get_external_session(session_id)
        if not external:
            return ""
